...
// // Updated: Added "Try On" button near Add to Cart that navigates to /try-on
// // with the matching frame preselected (same logic as Products.jsx).
...
// // ---------------------------------------------------------------------------
// // Re-export / import the shared frame-matching utility.
// // If Products.jsx is in a sibling folder, adjust the path accordingly.
// // Alternatively, copy the function here to keep files self-contained.
// // ---------------------------------------------------------------------------
// const getFrameIdForProduct = (product) => {
//   if (!product) return '/glass1.png';
//   if (product.frameId)    return product.frameId;
//   if (product.glassImage) return product.glassImage;
//   return '/glass1.png';
// };
...
//   return (
//     <div className="flex items-center border border-gray-200 rounded-full overflow-hidden">
//       <button onClick={decrease} className="w-10 h-10 flex items-center justify-center hover:bg-gray-100 transition-colors text-gray-600" aria-label="Decrease quantity">
//         <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//           <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 12H4" />
//         </svg>
//       </button>
//       <span className="w-12 text-center font-medium text-gray-800">{quantity}</span>
//       <button onClick={increase} className="w-12 h-12 flex items-center justify-center hover:bg-gray-100 transition-colors text-gray-600" aria-label="Increase quantity">
//         <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//           <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
//         </svg>
//       </button>
...
//             <label
//               key={variant.colorName}
//               className={`relative flex items-center gap-3 px-4 py-2 rounded-full border cursor-pointer transition-all duration-200 ease-out ${
//                 isSelected
//                   ? 'border-black bg-black text-white shadow-md'
//                   : 'border-gray-200 bg-white text-gray-700 hover:border-gray-400 hover:shadow-sm'
//               }`}
//             >
//               <input type="radio" name="productColor" value={variant.colorName} checked={isSelected} onChange={() => setSelectedVariant(variant)} className="sr-only" />
//               <div className="w-5 h-5 rounded-full border border-white/30 shadow-sm" style={{ backgroundColor: variant.hex || '#E8F4F8' }} />
//               <span className="text-sm font-medium">{variant.colorName}</span>
//               {isSelected && (
//                 <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//                   <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
...
// // ==================== ScrollNumberInput ====================
// const ScrollNumberInput = ({
//   value, onChange, min = -20, max = 20, step = 0.25,
//   label, unit = '', required = false, placeholder = '0.00',
//   description = '', showSign = true, allowNegative = true,
// }) => {
//   const displayValue = value === '' || value === undefined || value === null ? '' : value;
...
//     let nv  = Math.round((cur + step) / step) * step;
...
//       </label>
//       {description && <p className="text-[10px] text-gray-400 mb-2">{description}</p>}
//       <div className="flex items-center gap-2">
//         <button type="button" onClick={handleDecrement} className="w-10 h-10 flex-shrink-0 flex items-center justify-center bg-gray-100 hover:bg-gray-200 active:bg-gray-300 rounded-lg transition-all duration-150 text-gray-700" aria-label={`Decrease ${label}`}>
//           <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M20 12H4" /></svg>
//         </button>
//         <div className={`relative flex-1 ${!isValid() && required ? 'ring-2 ring-red-500 rounded-lg' : ''}`}>
//           <input
//             type="text" value={getFormattedValue()} onChange={handleInputChange}
//             onWheel={e => e.preventDefault()} placeholder={placeholder}
//             className={`w-full text-center py-2.5 px-2 bg-white border rounded-lg focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent font-mono text-sm font-medium transition-all ${getValueColorClass()} ${!isValid() && required ? 'border-red-500' : 'border-gray-200 hover:border-gray-300'}`}
//           />
//           {unit && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 pointer-events-none font-medium">{unit}</span>}
//         </div>
//         <button type="button" onClick={handleIncrement} className="w-10 h-10 flex-shrink-0 flex items-center justify-center bg-gray-100 hover:bg-gray-200 active:bg-gray-300 rounded-lg transition-all duration-150 text-gray-700" aria-label={`Increase ${label}`}>
//           <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4" /></svg>
//         </button>
//       </div>
//       <div className="flex justify-between items-center mt-1.5">
//         <span className="text-[9px] text-gray-400 bg-gray-50 px-1.5 py-0.5 rounded-full">Range: {min} to {max}</span>
//         <span className="text-[9px] text-gray-400 bg-gray-50 px-1.5 py-0.5 rounded-full">Step: {step}</span>
//       </div>
//       {!isValid() && required && <p className="text-red-500 text-[10px] mt-1">Required</p>}
//     </div>
//   );
// };
...
// const PrescriptionForm = ({ show, onClose, onSave, existingPrescription = null }) => {
//   const [lensType, setLensType] = useState(existingPrescription?.lensType || 'standard');
//   const [rightEye, setRightEye] = useState(existingPrescription?.rightEye || { sphere: '', cylinder: '', axis: '', addition: '' });
//   const [leftEye,  setLeftEye]  = useState(existingPrescription?.leftEye  || { sphere: '', cylinder: '', axis: '', addition: '' });
//   const [errors, setErrors]     = useState({});
...
//   const isAxisRequired = (eye) =>
//     eye.cylinder !== '' && eye.cylinder !== undefined && eye.cylinder !== null && eye.cylinder !== 0;

//   const validateForm = () => {
//     const newErrors = {};
//     if (isAxisRequired(rightEye) && (!rightEye.axis || rightEye.axis === '')) newErrors.right_axis = 'Axis is required when Cylinder has a value';
//     if (isAxisRequired(leftEye)  && (!leftEye.axis  || leftEye.axis  === '')) newErrors.left_axis  = 'Axis is required when Cylinder has a value';
//     setErrors(newErrors);
//     return Object.keys(newErrors).length === 0;
//   };
...
//   const handleRightChange = (field, value) => {
//     setRightEye(prev => ({ ...prev, [field]: value }));
//     if (field === 'cylinder' && (!value || value === 0)) setErrors(prev => ({ ...prev, right_axis: '' }));
//     if (field === 'axis' && errors.right_axis)           setErrors(prev => ({ ...prev, right_axis: '' }));
//   };
//   const handleLeftChange = (field, value) => {
//     setLeftEye(prev => ({ ...prev, [field]: value }));
//     if (field === 'cylinder' && (!value || value === 0)) setErrors(prev => ({ ...prev, left_axis: '' }));
//     if (field === 'axis' && errors.left_axis)            setErrors(prev => ({ ...prev, left_axis: '' }));
//   };

//   const handleSave = () => {
//     if (!validateForm()) return;
//     onSave({ lensType, lensName: selectedLens.name, lensPrice: selectedLens.price, rightEye, leftEye });
//     onClose();
//   };
...
//         {/* Lens Type */}
//         <div className="mb-6">
//           <label className="block text-sm font-bold text-gray-800 mb-2">Select Lens Type</label>
//           <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
//             {lensOptions.map(lens => (
//               <label key={lens.id} className={`relative p-3 rounded-lg border-2 cursor-pointer transition-all ${lensType === lens.id ? 'border-black bg-gray-50' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
//                 <input type="radio" name="lensType" value={lens.id} checked={lensType === lens.id} onChange={() => setLensType(lens.id)} className="sr-only" />
//                 <div className="flex justify-between items-start mb-1">
//                   <span className={`font-semibold text-sm ${lensType === lens.id ? 'text-black' : 'text-gray-800'}`}>{lens.name}</span>
//                   <span className={`font-bold text-sm ${lensType === lens.id ? 'text-black' : 'text-gray-700'}`}>+Pkr {lens.price}</span>
//                 </div>
//                 <p className="text-[10px] text-gray-500">{lens.description}</p>
//                 {lensType === lens.id && <div className="absolute top-2 right-2"><svg className="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg></div>}
//               </label>
//             ))}
//           </div>
//         </div>
...
//         {/* Right Eye */}
//         {[{ eye: rightEye, handler: handleRightChange, label: 'Right Eye', short: 'R', sub: 'OD (Oculus Dexter)', errKey: 'right_axis' },
//           { eye: leftEye,  handler: handleLeftChange,  label: 'Left Eye',  short: 'L', sub: 'OS (Oculus Sinister)', errKey: 'left_axis' }
//         ].map(({ eye, handler, label, short, sub, errKey }) => (
//           <div key={short} className="mb-6 bg-gray-50 rounded-xl border border-gray-200 p-4">
//             <div className="flex items-center gap-2 mb-4 pb-2 border-b border-gray-200">
//               <div className="bg-blue-600 w-8 h-8 rounded-lg flex items-center justify-center">
//                 <span className="text-white font-bold text-sm">{short}</span>
//               </div>
//               <div>
//                 <h4 className="font-bold text-base text-gray-800">{label}</h4>
//                 <p className="text-[10px] text-gray-500">{sub}</p>
//               </div>
//             </div>
//             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
//               <ScrollNumberInput label="Sphere (Power)" value={eye.sphere === '' ? '' : parseFloat(eye.sphere) || 0} onChange={v => handler('sphere', v)} min={-20} max={20} step={0.25} unit="D" placeholder="0.00" description="Distance correction (±)" showSign allowNegative />
//               <ScrollNumberInput label="Cylinder" value={eye.cylinder === '' ? '' : parseFloat(eye.cylinder) || 0} onChange={v => handler('cylinder', v)} min={-6} max={6} step={0.25} unit="D" placeholder="0.00" description="Astigmatism (±)" showSign allowNegative />
//               <div>
//                 <ScrollNumberInput label="Axis" value={eye.axis === '' ? '' : parseFloat(eye.axis) || 0} onChange={v => handler('axis', v)} min={0} max={180} step={1} unit="°" required={isAxisRequired(eye)} placeholder="0" description="0-180 degrees" showSign={false} allowNegative={false} />
//                 {errors[errKey] && (
//                   <p className="text-red-500 text-[10px] mt-1 flex items-center gap-1">
...
//                 )}
//               </div>
//               <ScrollNumberInput label="Addition" value={eye.addition === '' ? '' : parseFloat(eye.addition) || 0} onChange={v => handler('addition', v)} min={0} max={3.5} step={0.25} unit="D" placeholder="0.00" description="Reading/Multifocal (+ only)" showSign allowNegative={false} />
//             </div>
//           </div>
//         ))}
...
//         <div className="flex gap-3">
//           <button onClick={onClose}   className="flex-1 bg-gray-100 text-gray-700 py-2.5 rounded-lg font-semibold text-sm hover:bg-gray-200 transition-colors">Cancel</button>
//           <button onClick={handleSave} className="flex-1 bg-black text-white py-2.5 rounded-lg font-semibold text-sm hover:bg-gray-800 transition-colors">Save Prescription</button>
//         </div>
//       </div>
//     </div>
//   );
...
// // ==================== TryOnButton ====================
// // Standalone "Try On" button used in the product actions area.
// const TryOnButton = ({ onClick }) => (
//   <button
//     onClick={onClick}
//     aria-label="Virtually try on this frame"
//     style={{
//       display: 'flex',
//       alignItems: 'center',
...
//         animation: 'shimmerTryOn 2.4s ease-in-out infinite',
//         pointerEvents: 'none',
//       }}
//     />
//   </button>
...
//   // ── Prevent blank-state on direct navigation ─────────────────────────────
//   useEffect(() => {
//     const hasReloaded = sessionStorage.getItem('productDetailReloaded');
//     if (!hasReloaded) {
//       sessionStorage.setItem('productDetailReloaded', 'true');
...
//       return;
//     }
//     const t = setTimeout(() => sessionStorage.removeItem('productDetailReloaded'), 500);
...
//           if (Array.isArray(v.images)) allImages.push(...v.images);
//         });
//       }
...
//         madeInTaiwan:  found.madeInTaiwan || false,
//         originalPrice: parseFloat(found.originalPrice.toString().replace(/,/g, '')),
//         discountPrice: parseFloat(found.discountPrice.toString().replace(/,/g, '')),
//         reviews:       found.reviews || 0,
//         rating:        4.5,
//         description:   found.detailDescription || found.description || `Experience style and comfort with our ${found.name}.`,
//         features:      found.features || ['Premium quality material','UV protection coating','Scratch resistant','Lightweight design','Comfort fit'],
//         specifications: found.specifications || {
//           Material: found.type || 'Premium Plastic/Metal',
...
//         images:       allImages,
//         category:     found.category || 'eyeglasses',
//         inStock:      found.inStock !== undefined ? found.inStock : true,
//         freeShipping: found.freeShipping !== undefined ? found.freeShipping : true,
//         warranty:     found.warranty || '30-day satisfaction guarantee',
//         shape:        found.shape || 'Standard',
//         gender:       found.gender || 'Unisex',
//         code:         found.code,
//         color:        found.color,
//         pattern:      found.pattern,
//         // ── Frame-matching fields ────────────────────────────────────────
//         frameId:      found.frameId      || null,  // explicit mapping (preferred)
//         glassImage:   found.glassImage   || null,  // direct glass image path
//       };

//       setProduct(transformed);
//       if (transformed.variants?.length) {
//         setSelectedVariant(transformed.variants[0]);
//         if (transformed.variants[0]?.images?.[0]) setCurrentImage(transformed.variants[0].images[0]);
//       } else if (transformed.images?.[0]) {
//         setCurrentImage(transformed.images[0]);
//       }
//       setLoading(false);
//     } catch (err) {
//       console.error('Error loading product:', err);
...
//   const handleThumbnailClick = (img, idx) => { setCurrentImage(img); setActiveImageIndex(idx); };

//   // ── Price helpers ─────────────────────────────────────────────────────────
//   const getFramePrice    = ()  => product?.discountPrice || product?.originalPrice || 0;
//   const getLensExtraCharge = () => prescriptionData?.lensPrice || 0;
//   const getTotalItemPrice  = () => getFramePrice() + getLensExtraCharge();
...
//       framePrice: getFramePrice(), lensExtraCharge: getLensExtraCharge(),
//       totalPrice: getTotalItemPrice(), quantity,
//       selectedVariant, image: currentImage,
//       prescription: prescriptionData || null,
//     };
//     const idx = existing.findIndex(
//       i => i.id === cartItem.id &&
//            i.selectedVariant?.colorName === cartItem.selectedVariant?.colorName &&
//            i.prescription?.lensType === cartItem.prescription?.lensType
//     );
//     if (idx !== -1) existing[idx].quantity += quantity;
...
//   const handleAddToCart = () => addToCart();
//   const handleBuyNow    = () => { addToCart(); navigate('/checkout'); };

//   // ── Try On handler ────────────────────────────────────────────────────────
//   // Navigates to /try-on with the matched frameId in router state.
//   const handleTryOn = () => {
//     if (!product) return;
...
//         <div className="text-center">
//           <h2 className="text-2xl font-bold mb-4">{error || 'Product Not Found'}</h2>
//           <button onClick={() => navigate('/')} className="bg-black text-white px-6 py-3 rounded-full">Back to Home</button>
//         </div>
//       </div>
//     );
//   }
...
//       <div className="fixed inset-0 -z-10 pointer-events-none">
//         <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-purple-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-float" />
//         <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-blue-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-float-delayed" />
//       </div>
...
//           <button onClick={() => navigate('/')} className="hover:text-black transition-colors capitalize">{product.category?.replace('-', ' ') || 'Shop'}</button>
//           <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" /></svg>
//           <span className="text-gray-900 font-medium truncate">{product.name}</span>
//         </nav>
//       </div>
...
//                     key={idx}
//                     onClick={() => handleThumbnailClick(img, idx)}
//                     className={`w-20 h-20 rounded-xl overflow-hidden border-2 transition-all ${activeImageIndex === idx ? 'border-black shadow-md' : 'border-transparent opacity-70 hover:opacity-100'}`}
//                   >
//                     <img src={`.${img}`} alt={`View ${idx + 1}`} className="w-full h-full object-cover" />
//                   </button>
//                 ))}
...
//             <div className="flex flex-wrap gap-2">
//               {product.discount && parseFloat(product.discount) > 0 && (
//                 <span className="bg-red-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-sm">{discountPercent}% OFF</span>
//               )}
//               {product.madeInTaiwan && (
//                 <span className="bg-blue-100 text-blue-700 text-xs font-bold px-3 py-1 rounded-full shadow-sm">🇹🇼 Made in Taiwan</span>
//               )}
//               {product.freeShipping && (
//                 <span className="bg-green-100 text-green-700 text-xs font-bold px-3 py-1 rounded-full shadow-sm">Free Shipping</span>
//               )}
//               {product.inStock
//                 ? <span className="bg-emerald-100 text-emerald-700 text-xs font-bold px-3 py-1 rounded-full shadow-sm">In Stock</span>
//                 : <span className="bg-red-100 text-red-700 text-xs font-bold px-3 py-1 rounded-full shadow-sm">Out of Stock</span>
//               }
//             </div>

//             {/* Title */}
//             <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 leading-tight tracking-tight">{product.name}</h1>
...
//               <ColorVariantSelector variants={product.variants} selectedVariant={selectedVariant} setSelectedVariant={setSelectedVariant} />
//             )}
...
//                 <div className="flex-1">
//                   <p className="text-sm text-blue-800 font-medium">Prescription Lenses (Extra Charge)</p>
//                   <p className="text-xs text-blue-600 mt-1">Add your prescription details and choose lens type.</p>
//                   <button onClick={() => setShowPrescriptionModal(true)} className="text-xs text-blue-700 font-semibold underline mt-2 hover:text-blue-900">
//                     {prescriptionData ? 'Edit Prescription →' : 'Add Prescription →'}
//                   </button>
//                   {prescriptionData && (
//                     <div className="mt-2 text-xs text-green-700 bg-green-50 p-2 rounded-lg">
//                       <p>✓ {prescriptionData.lensName} added (+Pkr {prescriptionData.lensPrice})</p>
//                       <p className="text-gray-600 mt-1">
//                         {prescriptionData.rightEye.sphere !== '' ? `Right: ${prescriptionData.rightEye.sphere > 0 ? '+' : ''}${prescriptionData.rightEye.sphere}D` : 'Right: Not specified'}
//                         {' | '}
//                         {prescriptionData.leftEye.sphere !== '' ? `Left: ${prescriptionData.leftEye.sphere > 0 ? '+' : ''}${prescriptionData.leftEye.sphere}D` : 'Left: Not specified'}
//                       </p>
//                     </div>
//                   )}
//                 </div>
...
//               {/* Row 2: Try On – full-width, visually distinct */}
//               <div className="flex items-center gap-3">
//                 <TryOnButton onClick={handleTryOn} />
//                 <span className="text-xs text-gray-400 leading-tight">
//                   See how this frame looks<br />on your face in real-time
//                 </span>
//               </div>
//             </div>
...
//             <div className="flex flex-wrap gap-6 pt-4 border-t border-gray-100">
//               {[
//                 { icon: 'M5 13l4 4L19 7', label: '30-Day Returns' },
...
//                   <svg className="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={icon} /></svg>
//                   <span>{label}</span>
//                 </div>
//               ))}
//             </div>
...
//             <div className="flex gap-8 overflow-x-auto scrollbar-hide">
//               {['Features', 'Specifications', 'Shipping & Returns'].map((tab, idx) => (
//                 <button key={tab} className={`pb-4 text-lg font-medium transition-colors whitespace-nowrap ${idx === 0 ? 'text-black border-b-2 border-black' : 'text-gray-500 hover:text-black'}`}>
//                   {tab}
//                 </button>
//               ))}
//             </div>
...
//         @keyframes floatDelayed   { 0%,100%{transform:translate(0,0) scale(1)} 33%{transform:translate(-30px,30px) scale(1.1)} 66%{transform:translate(20px,-20px) scale(0.9)} }
//         @keyframes fadeSlideUp    { from{opacity:0;transform:translateY(40px)} to{opacity:1;transform:translateY(0)} }
//         @keyframes fadeIn         { from{opacity:0} to{opacity:1} }
//         @keyframes shimmerTryOn   { 0%{left:-75%} 50%,100%{left:125%} }
//         .animate-float            { animation:float 20s ease-in-out infinite; }
//         .animate-float-delayed    { animation:floatDelayed 25s ease-in-out infinite; }
//         .animate-slide-up         { animation:fadeSlideUp 0.3s ease-out forwards; }
//         .animate-fade-in          { animation:fadeIn 0.2s ease-out forwards; }
